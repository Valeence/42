...
export class WebSocketService {
  ...
  constructor(port: number = 8001, host: string = "0.0.0.0") {
    ...
    AppLogger.log(`🎮 WebSocket Signaling Server running on ${host}:${port}`);
  ...
  private handleConnection(ws: WebSocket): void {
    AppLogger.log("🔗 New signaling connection");
    ...
    ws.on("message", (data: Buffer) => {
      try {
        ...
        AppLogger.error("❌ Invalid message format:", error);
      }
    });
  ...
  private handleVoluntaryDisconnect(ws: WebSocket, message: any): void {
    ...
    AppLogger.log(
      `🚪 Player ${playerId} voluntarily disconnecting from match ${matchId}: ${reason}`,
    );
  ...
  private handleDisconnection(ws: WebSocket): void {
    ...
    AppLogger.log(`👋 Player ${playerId} disconnected`);
  ...
  private notifyOpponentDisconnection(playerId: string): void {
    for (const match of this.matches.values()) {
      if (
        ...
        match.guest.ws.readyState === WebSocket.OPEN
      ) {
        AppLogger.log(`📢 Notifying guest that host ${playerId} disconnected`);
        match.guest.ws.send(
          JSON.stringify({
        ...
        match.host.ws.readyState === WebSocket.OPEN
      ) {
        AppLogger.log(`📢 Notifying host that guest ${playerId} disconnected`);
        match.host.ws.send(
          JSON.stringify({
            type: "opponent_disconnected",
            reason: "guest_left",
